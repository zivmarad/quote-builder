...
export function buildSummaryHtml(totals: QuoteTotalsDisplay, vatLabel: string): string {
  const {
...
export function buildQuoteContent(params: {
  items: BasketItem[];
  totalBeforeVAT: number;
  totalWithVAT: number;
...
export const ROWS_PER_PAGE = 20; // שורות טבלה לכל עמוד (משאיר מקום לסיכום וחתימות בדף האחרון)

export function rowToHtml(item: BasketItem): string {
  const extrasTotal = item.extras?.reduce((sum, extra) => sum + extra.price, 0) || 0;
  const calculatedPrice = item.basePrice + extrasTotal;
  const currentPrice = item.overridePrice ?? calculatedPrice;
  const qty = item.quantity ?? 1;
