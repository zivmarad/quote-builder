...
משתמשים ונתונים (פרופיל, סל, היסטוריה, לקוחות CRM) נשמרים ב-Supabase. **כדי שפרטי הפרופיל (שם, טלפון, לוגו) יישמרו בין כניסות ומכשירים** – הגדר ב-Vercel את `NEXT_PUBLIC_SUPABASE_URL` ו־`SUPABASE_SERVICE_ROLE_KEY`, והרץ את `supabase-schema.sql` ב-Supabase (SQL Editor). ל**לקוחות (CRM)** אחרי מערכת משתמשים קיימת, הרץ גם `supabase-customers.sql` (או עדכן סכמה מלאה מ־`supabase-schema.sql`). לספירת קליקים בדשבורד המנהל הרץ גם `supabase-product-events.sql`. התחברות מכל מכשיר עם אותו חשבון מסנכרנת את הנתונים אוטומטית.
