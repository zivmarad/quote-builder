...
### לוגואים יציבים (מומלץ)

כדי שהלוגו יישמר בשרת כקובץ (ולא רק כ־base64 כבד ב־localStorage), הרץ ב־**SQL Editor** גם את הקובץ **`supabase-storage-profile-logos.sql`** (יוצר bucket `profile-logos` ומדיניות קריאה ציבורית). בלי זה – העלאת לוגו מהאפליקציה תישאר רק ב־localStorage (ללא קובץ בשרת).
